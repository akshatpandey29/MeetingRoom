...
function ToastContainer({ toasts }) {
  return (
    <div style={{ position:'fixed', top:20, right:20, zIndex:9999, display:'flex', flexDirection:'column', gap:10, pointerEvents:'none' }}>
      {toasts.map((t) => (
        <div key={t.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 18px', background: t.type==='success'?'#0f172a':'#991b1b', color:'#fff', borderRadius:14, fontSize:13, fontWeight:500, boxShadow:'0 4px 24px rgba(0,0,0,0.18)', animation:'slideIn 0.3s ease', minWidth:240 }}>
          {t.type==='success' ? <FaCheckCircle size={14} style={{color:'#4ade80',flexShrink:0}}/> : <span style={{color:'#fca5a5',flexShrink:0}}>✕</span>}
          {t.message}
        </div>
      ))}
...
function Field({ icon, label, error, hasError, children }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:6, marginBottom:14 }}>
      <label style={{ fontSize:11, fontWeight:600, color:'#64748b', letterSpacing:'0.06em', textTransform:'uppercase' }}>{label}</label>
      <div style={{ display:'flex', alignItems:'center', gap:8, border: showBorder?'1.5px solid #ef4444':'1.5px solid #e2e8f0', borderRadius:10, padding:'11px 13px', background: showBorder?'#fff5f5':'#f8fafc', transition:'border-color 0.15s' }}>
        <span style={{ color: showBorder?'#ef4444':'#94a3b8', display:'flex', alignItems:'center', flexShrink:0 }}>{icon}</span>
        {children}
      </div>
      {error && error.trim() && <p style={{ fontSize:12, color:'#ef4444', margin:0 }}>{error}</p>}
    </div>
  );
}
...
function LeftPanel() {
  return (
    <div style={{ width:'100%', background:'#0f172a', display:'flex', flexDirection:'column', justifyContent:'space-between', padding:'40px 44px', position:'relative', overflow:'hidden' }}>
      <div style={{ position:'absolute', inset:0, backgroundImage:'radial-gradient(circle, #ffffff08 1px, transparent 1px)', backgroundSize:'24px 24px' }}/>
      <div style={{ position:'absolute', top:-80, right:-80, width:300, height:300, borderRadius:'50%', background:'radial-gradient(circle, #2563eb20, transparent 70%)' }}/>
...
function LoginPage() {
  ...
  const inputStyle = { flex:1, fontSize:13.5, color:'#0f172a', outline:'none', border:'none', background:'transparent' };
  ...
  const validateRegister = () => {
    ...
    else if(regData.password.length<8) { errs.password='Too short — minimum 8 characters required'; }
    else if(!/[A-Z]/.test(regData.password)) { errs.password='Add at least one uppercase letter'; }
    else if(!/[a-z]/.test(regData.password)) { errs.password='Add at least one lowercase letter'; }
    else if(!/[0-9]/.test(regData.password)) { errs.password='Add at least one number'; }
    else if(!/[!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?]/.test(regData.password)) { errs.password='Add at least one special character'; }
    return errs;
  };
